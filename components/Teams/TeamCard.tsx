...
const TeamCard = ({ name, role, instagram, email, github, imageUrl, linkedin }: TeamCardProps) => {
    return (
        <Card>
            <CardHeader>
            ...
            <CardContent>
                ...
                <CardDescription className={'text-muted '}>{role}</CardDescription>
            </CardContent>
            <CardFooter className={'flex items-center gap-3 justify-end mt-4'}>
                <Link href={email} className={cn(buttonVariants({ size: 'icon', variant: 'ghost' }), '')}><Mail
                    width={24}
                    height={24} /></Link>
                ...
                {github && <Link href={github}
                                 className={cn(buttonVariants({ size: 'icon', variant: 'ghost' }), '')}><GitHubLogoIcon
                    width={24} height={24} /></Link>}
