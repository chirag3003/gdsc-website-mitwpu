...
function Navbar() {
    ...
    return (
        <div
            className="fixed z-10 top-0 left-0 right-0 py-5 border-b bg-background px-8 lg:px-24">
            <div className="relative w-full flex items-center justify-between">
                <Link href={'/'} className="logo"><img src="/gdsc-logo.svg" alt="logo"
                    ...
                    {navLinks.map((link, index) => {
                        return (
                            <Link
                                key={index}
                                href={link.href}
                                className={'text-sm rounded hover:bg-secondary hover:text-secondary-foreground px-4 py-2'}
                            >
                                {link.title}
                            </Link>
                        )
